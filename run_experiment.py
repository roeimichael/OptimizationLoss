"""
Single Experiment Runner with Model Caching
Executes one experiment configuration with intelligent model weight caching
"""
import torch
...
def get_model_cache_path(base_model_id):
    """
    Get path to cached model weights

    Args:
        base_model_id: Unique identifier for the model configuration

    Returns:
        Path: Path to cached model file
    """
...
def save_model_to_cache(model, scaler, base_model_id, config):
    """
    Save model weights and scaler to cache after warmup training

    Args:
        model: Trained PyTorch model
        scaler: Fitted StandardScaler
        base_model_id: Unique identifier for caching
...
def load_model_from_cache(base_model_id, input_dim, config, device):
    """
    Load model weights and scaler from cache
    ...
    Returns:
        tuple: (model, scaler) or (None, None) if not cached
    """
    cache_path = get_model_cache_path(base_model_id)
...
def train_warmup_phase(model, train_loader, optimizer, criterion_ce, device, warmup_epochs):
    """
    Train model for warmup phase (pure cross-entropy, no constraints)
...
def train_constraint_phase(model, train_loader, X_test_tensor, group_ids_test,
                           optimizer, criterion_ce, criterion_constraint, device,
                           epochs, warmup_epochs, constraint_threshold, lambda_step):
    """
    Train model with constraint-based optimization

    Args:
        model: Pre-trained model from warmup
        train_loader: Training data loader
        ...
        device: PyTorch device
        epochs: Total epochs
        warmup_epochs: Warmup epochs already completed
        constraint_threshold: Threshold for constraint violations
        lambda_step: Step size for lambda updates
    ...
    for epoch in range(warmup_epochs, epochs):
        ...
        for batch_features, batch_labels in train_loader:
            ...
            # Get test predictions for constraint evaluation
            model.eval()
            with torch.no_grad():
...
def run_single_experiment(config_path):
    ...
    if model is None:
        ...
        # Model loaded from cache - prepare data with cached scaler
