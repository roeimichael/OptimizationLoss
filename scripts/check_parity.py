...
def main():
    ...
    if len(versions) > 1:
        ...
        # 🛑 NOT A WARN. `src/utils/gitver.py` returns "unknown" when git
        # is unavailable, and a single shared "unknown" SATISFIES this gate
        # and every other equality check -- including the model cache's
        # reuse test, which would then carry weights across a code change.
        # One value is not the same as one commit.
        fails.append("CODE VERSION IS `unknown`: git was unavailable when "
                     "these runs were stamped, so `one value` here means `no "
                     "information`, not `one commit`")
        print("   FAIL -- the stamp carries no information; do not compare "
              "these arms")
    elif "-dirty" in list(versions)[0]:
