...
MET = ["cc_f1", "F1 (Macro)"]
RIVALS = ["aug_clip", "aug_tralo_null", "aug_tralo_stab", "clip", "focal_clip", "tralo_null"]
FAMILIES = {"augfin": "MobileNetV3+aug"}
...
def load(roots):
    ...
    for root in roots:
        man = "%s/%s/campaign_manifest.json" % (BASE, root)
        if os.path.exists(man):
            rt = json.load(open(man)).get("runtime", {})
            hosts.add(rt.get("host")); prec.add(rt.get("precision"))
        for f in glob.glob("%s/%s/*/*/*/*/*/evaluation_metrics.csv" % (BASE, root)):
...
for fam, backbone in FAMILIES.items():
    ...
    for met in MET:
        print("")
        print("  %s" % met)
        print("  %-10s | %s" % ("cap", " | ".join("%-18s" % ("aug_tralo - " + r) for r in RIVALS)))
        print("  " + "-" * 100)
        for cap in caps:
            cells_out = []
            for r in RIVALS:
                v = paired(runs, cap, "aug_tralo", r, met)
                if v is None:
