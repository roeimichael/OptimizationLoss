"""Measure the additive-factor control on observed group metadata."""
...
def control(path, sep="|", n_null=200, seed=0):
    ...
    for g in sorted(te["location"].unique()):
        ...
        if f0 == f1:
            ...
            q = np.divide(
                p_f0[f0] * p_f1[f1], p_glob, out=np.zeros_like(p_glob), where=p_glob > 0
            )
            q = q / q.sum() if q.sum() > 0 else p_glob
        unseen_add.append((cc(te_g), q, len(te_g)))
    shift = np.divide(cc(te) / n_te, p_glob, out=np.ones_like(p_glob), where=p_glob > 0)
    ...
    return {
        "path": path,
        "sep": sep,
        "unseen": len(unseen_glob),
        "raked": n_raked,
        "no_sep": no_sep,
        "unseen_factor": unseen_factor,
        "multi_tok": multi_tok,
        ...
        "net_all_global": all_a,
        "net_all_additive": all_b,
        "survives_diluted": 100 * all_b / all_a if all_a and n_raked else float("nan"),
    }


def report(r, name):
    head = (
        "  %-24s unseen=%2d (%4.1f%% of test)  raked=%2d  NET(global) %+7.0f z=%5.1f"
        % (
            name[-24:],
            r["unseen"],
            100 * r["unseen_share"],
    ...
    if not r["raked"]:
        why = (
            "the separator %r does not occur in any unseen group label" % r["sep"]
            if r["no_sep"] == r["unseen"]
            else "every unseen group has a factor level that is itself unseen"
        )
        return [
            head,
    ...
    lines.append(
        "      (unseen groups only. Over the WHOLE slice it reads %.1f%%, diluted by the seen groups, which are identical in both arms)"
        % r["survives_diluted"]
    )
    if r["unseen_share"] > 0.5:
        lines.append(
            "      *** %.0f%% of the test set is unseen, so the GLOBAL shift is largely computed from these very groups and corrects the raked baseline twice. Read the item counts, not the ratio."
            % (100 * r["unseen_share"])
        )
    if r["raked"] < r["unseen"]:
        lines.append(
            "      (%d of %d unseen groups kept the GLOBAL baseline: %d no separator, %d unseen factor level)"
            % (r["unseen"] - r["raked"], r["unseen"], r["no_sep"], r["unseen_factor"])
        )
    if r["multi_tok"]:
        lines.append(
            "      (%d raked group(s) have >2 tokens; only the FIRST and LAST are used, the middle is dropped)"
            % r["multi_tok"]
        )
    return lines


def _synthetic(tmp, sep="|", n_class=4, seed=0, n_tr=900, n_unseen=600, n_seen=1200):
    rng = np.random.default_rng(seed)
    cells = {
        (0, 0): rng.dirichlet(np.ones(n_class) * 0.7),
    ...
    for (s, a), p in cells.items():
        tr += [
            {"location": "s%d%sa%d" % (s, sep, a), "label": int(c)}
    ...
    q = q / q.sum()
    te = [
        {"location": "s1%sa1" % sep, "label": int(c)}
        for c in rng.choice(n_class, size=n_unseen, p=q)
    ]
    for cell in ((0, 0), (1, 0)):
        te += [
            {"location": "s%d%sa%d" % (cell[0], sep, cell[1]), "label": int(c)}
...
def self_test():
    ...
    ok = True
    live = [
        control(_synthetic(os.path.join(root, "f%d" % s), seed=s), sep="|")
        for s in range(6)
    ]
    ...
    if any((r["raked"] != r["unseen"] or not r["raked"] for r in live)):
        ...
        print(
            "FAIL (a): raking absorbed only %.0f%% of a slice built so that raking is exact"
            % (100 - surv)
        )
        ok = False
    else:
        print(
            "  PASS (a) LIVENESS: the control RUNS and measures -- survives %.1f%% over 6 seeds on a slice where raking is exact"
            % surv
        )
    if not dil > surv + 10.0:
        print(
            "FAIL (b): expected the whole-slice figure (%.1f%%) to sit well above the unseen-only one (%.1f%%)"
            % (dil, surv)
        )
        ok = False
    else:
        print(
            "  PASS (b) DILUTION: whole-slice %.1f%% vs unseen-only %.1f%% -- the headline is the undiluted number"
            % (dil, surv)
        )
    ...
    print(txt)
    if dead["raked"] != 0:
        print("FAIL (c): a wrong separator still raked %d group(s)" % dead["raked"])
        ok = False
        ...
        print(
            "  PASS (c) REFUSAL: the wrong separator is REFUSED, not reported as ~100%"
        )
    if dead["net_global"] != dead["net_additive"]:
        print(
            "FAIL (d): expected the un-raked arms to coincide EXACTLY, got %+.4f vs %+.4f"
            % (dead["net_global"], dead["net_additive"])
        )
        ok = False
    else:
        print(
            "  PASS (d) the un-raked arms are bit-identical (%+.0f items both), which is why ~100%% could never fail"
            % dead["net_global"]
        )
