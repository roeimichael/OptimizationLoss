...
def write_dump(path, names, rows):
    ...
    with io.open(path, "w", encoding="utf-8", newline="") as fh:
        ...
        for i, nm in enumerate(names):
            ...
            # `cell` now carries the arm as its 4th element and the arm is
            # written as its own column below, so take only the first three.
            w.writerow([nm] + list(cell[:3]) + [arm, seed] +
