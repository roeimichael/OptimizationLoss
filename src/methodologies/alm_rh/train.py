"""alm_rh methodology: the ALM dual + TraLO's two load-bearing components.

REVIEW-RESPONSE GRAFT (blind review round 1, Pillar 2). The reviewer's point:
the paper's thesis is that the optimizer reset and the undershoot hinge carry
the effect, yet ALM -- the strongest comparator -- is the one baseline that
never received them. Comparing against an un-upgraded opponent proves less than
it appears to.

This arm is `fioretto_rh` with the subgradient dual replaced by the ALM rule of
`fioretto_alm`:

    lambda_c <- max(0, lambda_c + eta (S_c - K_c)) + mu_t (S_c - K_c)^+
    mu_t = alm_mu0 + alm_mu_step * epoch

so it differs from `fioretto_alm` in exactly the two grafted components, and
from `fioretto_rh` in exactly the dual update. Both diffs are one-dimensional,
which is what makes the three-way comparison interpretable.

Generated by scripts; do not hand-edit -- regenerate if fioretto_rh changes.
"""
...
def _train_constraints(model, config, inputs, device):
    ...
    # --- ALM dual knobs (identical to src/methodologies/fioretto_alm) ---
    ...
    log.info("ALM-RH (graft): %d epochs, lr=%.2e, eta=%.4f, beta=%.2f, "
             "reset_at_sat=%s, %d global + %d local multipliers",
             constraint_epochs, lr_c, eta, fior_beta, reset_optimizer_at_sat,
    ...
    for epoch in range(constraint_epochs):
        ...
        violations_g = {}
        residual_g = {}   # signed, for the ALM ascent term
        violated_global = set()
        for c in constrained_classes:
            K = global_con[c]
            if K >= UNLIMITED:
        ...
        violations_l = {}
        residual_l = {}   # signed, for the ALM ascent term
        violated_local = set()
        for g in unique_groups:
        ...
        # ---- Step 3: augmented-Lagrangian dual update (fioretto_alm, verbatim) ----
        ...
        if all_satisfied and satisfaction_epoch is None:
            ...
            log.info("ALM-RH: first satisfaction at epoch %d", epoch + 1)
