...
def _train_constraints(model, inputs: TrainInputs, device):
    ...
    if abs(1.0 - 2.0 * eta_u * alpha) >= 1.0:
        raise ValueError(
            f"hounie_rcl: eta_u={eta_u} with alpha={alpha} gives stability factor {1.0 - 2.0 * eta_u * alpha:+.3f}; |factor| >= 1 means the perturbation u oscillates or diverges instead of converging to lambda/(2*alpha). arXiv:2306.02426 App. F uses eta_u=0.1 with h(u)=||u||^2 (alpha=1), i.e. factor 0.8."
        )
    ...
    log.info(
        "Hounie RCL: %d constraint epochs, lr=%.2e eta_lam=%.4f eta_u=%.4f alpha=%.2f, %d global + %d local constraints",
        constraint_epochs,
        lr_c,
        eta_lambda,
        eta_u,
    ...
    for epoch in range(constraint_epochs):
        ...
        for key in K_local:
            u_l[key] = max(
                0.0, u_l[key] + eta_u * (lam_l[key] - 2.0 * alpha * u_l[key])
            )
        constraint_loss_val = 0.0
        has_active = any((v > 0 for v in lam_g.values())) or any(
            (v > 0 for v in lam_l.values())
        )
        did_backward = False
        if has_active:
            ...
            for i in range(0, n_test, chunk_size):
                with constraint_autocast(amp_dtype, use_amp, step_cfg["fp32"]):
                    ...
                    for c, lam in lam_g.items():
                        if lam > 0:
                            chunk_loss = (
                                chunk_loss + lam * chunk_proba[:, c].sum() / n_test
                            )
                    ...
                    for (g, c), lam in lam_l.items():
                        if lam > 0:
                            ...
                            if mask.any():
                                N_g = max(1, group_sizes[g])
                                chunk_loss = (
                                    chunk_loss + lam * chunk_proba[mask, c].sum() / N_g
                                )
        ...
        h_u = alpha * (
            sum((v**2 for v in u_g.values())) + sum((v**2 for v in u_l.values()))
        )
        row = {
            "epoch": epoch,
        ...
        if epoch < 5 or (epoch + 1) % 10 == 0 or epoch == constraint_epochs - 1:
            ...
            log.info(
                "Hounie %d/%d: CE=%.4f cstr=%.4f excess=%d sat=%s stable=%d lam=[%s] u=[%s] h_u=%.4f [%.1fs]",
                epoch + 1,
                constraint_epochs,
                np.mean(ce_losses),
                constraint_loss_val,
