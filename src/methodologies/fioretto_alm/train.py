"""Augmented-Lagrangian dual update with projected residual ascent.
...
The two-pass constraint gradient, task phase and satisfaction tracking share
the Fioretto-LDF implementation conventions.
"""
...
def _train_constraints(model, inputs, device):
    ...
    log.info(
        "Fioretto ALM: %d epochs, lr=%.2e, eta=%.4f, mu0=%.4f, mu_step=%.4f, %d global + %d local multipliers",
        constraint_epochs,
        lr_c,
        eta,
        mu0,
    ...
    for epoch in range(constraint_epochs):
        ...
        has_work = any(
            (lambda_g.get(c, 0) + aug_g.get(c, 0.0) > 0 for c in violated_global)
        ) or any((lambda_l.get(k, 0) + aug_l.get(k, 0.0) > 0 for k in violated_local))
                ...
                with constraint_autocast(amp_dtype, use_amp, step_cfg["fp32"]):
                    ...
                    for key in violated_local:
                        ...
                        if w_l > 0:
                            ...
                            if mask.any():
                                chunk_loss = (
                                    chunk_loss + w_l * chunk_proba[mask, c].sum()
                                )
        ...
        if epoch < 5 or (epoch + 1) % 25 == 0 or epoch == constraint_epochs - 1:
            ...
            log.info(
                "Fioretto ALM %d/%d: CE=%.4f cstr=%.4f excess=%d sat=%s stable=%d mu=%.3f lam=[%s] [%.1fs]",
                epoch + 1,
                constraint_epochs,
                np.mean(ce_losses),
                constraint_loss_val,
