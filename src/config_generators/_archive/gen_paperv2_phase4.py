"""Phase 4 of paper plan v2 — Table D (multi-class robustness on dermmnist).

Grid:
    DermMNIST × MobileNetV3 × cls ∈ {0 AKIEC, 1 BCC, 2 BKL} × group=loc_group
    × 5 symmetric tightness × 6 methods × 4 seeds = 360 target cells.

cls=4 (MEL) is the story class already covered by Phase 1 (Table A), so it
is excluded here. DF (cls=3) and VASC (cls=6) are excluded — too rare,
K collapses to 0 under tight constraints.

New configs land at
    results/pending_runs/paperv2_phase4/cls_{c}/{tight}/{method}/seed_{s}/
"""
...
SWEEP_ROOT = "results/pending_runs/paperv2_phase4"
...
def make_cfg(cls, tight_tag, method, seed):
    ...
    return {
        ...
        "exp_name": f"paperv2_p4_{method}_{DS_NAME}_cls{cls}_{tight_tag}_seed{seed}",
        "experiment_path": str(
            Path(SWEEP_ROOT) / f"cls_{cls}" / tight_tag / method / f"seed_{seed}"
        ),
    }
    ...
    for cls in CLASSES:
        for tight in TIGHTNESS:
            for method in METHODS:
                for seed in SEEDS:
                    key = (DS_NAME, MODEL, cls, "loc_group", tight, method, seed)
                    if key in done:
                        skipped += 1
                        continue
