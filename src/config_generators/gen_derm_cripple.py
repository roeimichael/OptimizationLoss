...
ranking on AIDER. This experiment validates the mechanism on DermMNIST
with clinically-realistic corruptions.

Corruption types (both train AND test corrupted; same seed across types
for paired-comparison fairness):
  ...
  color:   HSV jitter (hue=0.15, sat=0.4, bri=0.25) - scanner variation
  defocus: Gaussian blur sigma=2.5 - out-of-focus capture

Grid:
  5 corruptions x 3 tightness (L20, L30, L50) x 4 methods x 2 seeds = 120 cells.
...
CORRUPTIONS = {
    ...
    "defocus": "data/dermmnist_defocus/slice_1",
}
