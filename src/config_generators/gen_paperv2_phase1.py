"""Phase 1 of paper plan v2 — fill missing Table A cells.

Headline method comparison:
    3 datasets × MobileNetV3 × story-(cls,grp) × 5 symmetric tightness
    × 6 methods × 4 seeds = 360 target cells.

Detects already-completed cells by scanning all of
`results/pending_runs/**/evaluation_metrics.csv` and skips them.
New configs land at
    results/pending_runs/paperv2_phase1/{ds}/{tight}/{method}/seed_{s}/
"""
...
SWEEP_ROOT = "results/pending_runs/paperv2_phase1"
...
def _scan_done_cells():
    """Return set of (ds, model, cls, grp, tight, method, seed) for
    every cell that already has an evaluation_metrics.csv anywhere
    under results/pending_runs/."""
    done = set()
...
def make_cfg(ds_name, ds_meta, tight_tag, method, seed):
    ...
    return {
        ...
        "hyperparams": hp,
        "base_model_id": bmid,
        "exp_name": f"paperv2_p1_{method}_{ds_name}_{tight_tag}_seed{seed}",
        "experiment_path": str(
            Path(SWEEP_ROOT) / ds_name / tight_tag / method / f"seed_{seed}"
        ),
    }
...
def build():
    ...
    print(f"Target: 360 cells. Already done elsewhere: {skipped}. "
