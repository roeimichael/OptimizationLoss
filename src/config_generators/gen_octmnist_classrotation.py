"""OctMNIST class-rotation sweep: show TraLO win is not specific to the c2
(drusen) constrained class — sweep alternative cap classes (c0/c1/c3) on the
headline backbone (MobileNetV3). Mirrors the class-rotation grids we ran for
tissue/derm/aider.

  MobileNetV3 only
  3 alt constrained classes (0, 1, 3)
...
MODEL = "MobileNetV3"
ALT_CLASSES = [0, 1, 3]  # all except the existing c2 baseline
...
def make_cfg(cls, tight, method, seed):
    ds_config = {
        ...
        "group_column": GROUP_COLUMN, "constrained_class": cls,
        "data_dir": DATA_DIR,
    }
    ...
    return {
        ...
        "experiment_path": (
            f"results/pending_runs/octmnist_classrotation/c{cls}/{MODEL}/{tight}/{method}/seed_{seed}"
        ),
    }
...
def main():
    ...
    print(f"Generated {len(cfgs)} configs ({len(ALT_CLASSES)} classes x "
