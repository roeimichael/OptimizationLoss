"""Phase 3 of paper plan v2 — Table C (backbone robustness on dermmnist).

Grid:
    DermMNIST × {ResNet18, EfficientNetB0} × cls=4 (MEL) × group=loc_group
    × 5 symmetric tightness × 6 methods × 4 seeds = 240 target cells.

MobileNetV3 is already covered by Phase 1 (Table A) so it is excluded here
to avoid duplication.

Already-completed cells are detected by scanning all of
`results/pending_runs/**/evaluation_metrics.csv` and skipped.
New configs land at
    results/pending_runs/paperv2_phase3/{backbone}/{tight}/{method}/seed_{s}/
"""
...
SWEEP_ROOT = "results/pending_runs/paperv2_phase3"
...
def make_cfg(backbone, tight_tag, method, seed):
    ...
    return {
        ...
        "hyperparams": hp,
        "base_model_id": bmid,
        "exp_name": f"paperv2_p3_{backbone}_{method}_{DS_NAME}_{tight_tag}_seed{seed}",
        "experiment_path": str(
