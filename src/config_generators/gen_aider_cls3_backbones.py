"""Backbone-robustness sweep on AIDER cls 3 majority — the TraLO win cell.

Tests whether the d_F1 +0.05 vs LP+heuristic generalizes across the 4 paper
backbones (MobileNetV3, MobileNetV2, RegNetY400MF, ShuffleNetV2). If yes, the
finding is paper-quality. If MobileNetV3-only, narrower claim.

Layout: 1 dataset × 1 class (3) × 4 backbones × 5 methods × 5 seeds × L30_G30 = 100 cells.
"""
...
def make_cfg(backbone, seed, method):
    ds_config = {
        ...
        "group_column": "synth_group", "constrained_class": CONSTRAINED_CLASS,
    ...
    bmid = compute_base_model_id(
        backbone, hp, dataset_mode="aider", data_dir="data/aider/slice_1",
        dataset_config=ds_config,
    )
    return {
        "methodology": method, "model_name": backbone,
        "constraint": list(pair), "constraint_tag": TIGHT,
        "dataset_mode": "aider", "dataset_config": ds_config,
        "hyperparams": hp, "base_model_id": bmid,
        "experiment_path": (
            f"results/pending_runs/aider_cls3_backbones/"
...
def main():
    cfgs = [
        make_cfg(b, s, m)
        for b in BACKBONES for s in SEEDS for m in METHODS
    ]
