#!/usr/bin/env python3
"""
Comparative Analysis Generator for Constraint Satisfaction Experiments

This script analyzes all experiment results and generates:
- Master CSV with all experiment data
- Constraint-specific folders with CSVs and 7 graphs each
- Cross-constraint comparison graph
...
def save_csv(experiments, output_path):
...
def plot_accuracy_by_learning_rate(experiments, output_dir, constraint_name, constraint_key):
    ...
    for i, model in enumerate(models):
        ...
        for lr in learning_rates:
            if lr in data[model]:
                avg_accuracies.append(np.mean(data[model][lr]))
            else:
    ...
    plt.tight_layout()
    plt.savefig(output_dir / 'accuracy_by_learning_rate.png', dpi=300, bbox_inches='tight')
    plt.close()
...
def plot_accuracy_by_lambda_strategy(experiments, output_dir, constraint_name, constraint_key):
    ...
    for i, model in enumerate(models):
        ...
        for strategy in strategies:
            if strategy in data[model]:
                avg_accuracies.append(np.mean(data[model][strategy]))
            else:
                avg_accuracies.append(0)
        ax.bar(x + i * width, avg_accuracies, width, label=model)
    ...
    ax.set_ylabel('Average Test Accuracy', fontsize=12)
    ax.set_title(f'Average Test Accuracy by Lambda Strategy - {constraint_name}', fontsize=14)
    ax.set_xticks(x + width)
    ...
    plt.tight_layout()
    plt.savefig(output_dir / 'accuracy_by_lambda_strategy.png', dpi=300, bbox_inches='tight')
    plt.close()


def plot_convergence_rate_by_factors(experiments, output_dir, constraint_name):
    ...
    axes[0].set_title('By Model', fontsize=12)
    ...
    axes[1].set_title('By Learning Rate', fontsize=12)
    ...
    axes[2].set_title('By Lambda Strategy', fontsize=12)
    ...
    plt.tight_layout()
    plt.savefig(output_dir / 'convergence_rate_by_factors.png', dpi=300, bbox_inches='tight')
    plt.close()
...
def plot_model_comparison(experiments, output_dir, constraint_name, constraint_key):
    ...
    ax1.set_title('Convergence Speed by Model', fontsize=12)
...
def plot_heatmap_lr_strategy(experiments, output_dir, constraint_name, constraint_key):
    ...
    for lr in lrs:
        ...
        for strategy in strategies:
            if strategy in data[lr] and len(data[lr][strategy]) > 0:
                row.append(np.mean(data[lr][strategy]))
            else:
                row.append(0)
        matrix.append(row)
    ...
    for i in range(len(lrs)):
        for j in range(len(strategies)):
            if matrix[i][j] > 0:
                ax.text(j, i, f'{matrix[i][j]:.3f}', ha="center", va="center", color="black", fontsize=10)

    cbar = plt.colorbar(im, ax=ax)
    cbar.set_label('Test Accuracy', fontsize=11)

    plt.tight_layout()
    plt.savefig(output_dir / 'heatmap_lr_strategy.png', dpi=300, bbox_inches='tight')
    plt.close()
...
def plot_accuracy_vs_convergence_speed(experiments, output_dir, constraint_name, constraint_key):
    ...
    plt.tight_layout()
    plt.savefig(output_dir / 'accuracy_vs_convergence_speed.png', dpi=300, bbox_inches='tight')
    plt.close()


def plot_lr_vs_convergence_epochs(experiments, output_dir, constraint_name):
    """Graph 7: Box plot of convergence epochs by learning rate."""
    converged = [e for e in experiments if e['converged']]
    if not converged:
        return
    ...
    ax.set_title(f'Convergence Epochs Distribution by Learning Rate - {constraint_name}', fontsize=14)
    ax.grid(axis='y', alpha=0.3)
    plt.xticks(rotation=45, ha='right')

    plt.tight_layout()
    plt.savefig(output_dir / 'lr_vs_convergence_epochs.png', dpi=300, bbox_inches='tight')
    plt.close()


def plot_cross_constraint_comparison(experiments, output_dir):
    """Generate cross-constraint comparison graph."""
    converged = [e for e in experiments if e['converged']]
    if not converged:
        return

    # Convergence speed by constraint
    ...
    ax2.bar(constraints, avg_accs, color=['#2ca02c', '#ff7f0e', '#d62728'])
    ax2.set_xlabel('Constraint Level (Global, Local)', fontsize=12)
    ax2.set_ylabel('Average Test Accuracy', fontsize=12)
    ax2.set_title('Test Accuracy by Constraint', fontsize=13)
    ax2.grid(axis='y', alpha=0.3)
    ...
    plt.tight_layout()
    plt.savefig(output_dir / 'cross_constraint_comparison.png', dpi=300, bbox_inches='tight')
    plt.close()
...
def main():
    ...
    print("\n[1/3] Extracting experiment data from results folder...")
    ...
    print(f"\n[3/3] Generating constraint-specific analyses ({len(constraints)} constraints)...")

    # Process each constraint
    for i, constraint in enumerate(constraints, 1):
        print(f"\n  [{i}/{len(constraints)}] Constraint: {constraint}")
        ...
        print(f"      Experiments: {len(constraint_exps)}")
        print(f"      Converged: {converged_count} ({converged_count/len(constraint_exps)*100:.1f}%)")

        # Create constraint directory
        constraint_dir = output_base / f'constraint_{constraint}'
