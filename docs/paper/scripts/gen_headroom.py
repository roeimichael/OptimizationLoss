...
TraLO early-stops on five consecutive satisfied epochs, so left alone it spends
far LESS than B. Measured on headroom_b30: TraLO logs 7.2-8.8 epochs of its 29
while fioretto_ldf logs 21.8-29.0, hounie_rcl 26.4-29.0, and the post-hoc arms
get all 30. That asymmetry was originally left in on the argument that winning
on less compute is a stronger claim. The argument is sound for a win and
backwards for a loss: TraLO trailing the clipper while it trains a quarter as
long is not evidence about the objective, it is evidence about epochs. So
--full-budget raises stable_count_threshold above the budget for EVERY trained
arm and all methods spend exactly B. Run both: the early-stop campaign prices
convergence speed, the full-budget campaign prices the loss.
...
def main():
    ...
    ap.add_argument("--full-budget", action="store_true",
                    help="disable convergence early-stopping for every trained "
                         "arm by raising stable_count_threshold above the "
                         "budget, so all methods spend the same epochs. "
                         "Without it TraLO exits at ~8 of 29 epochs (it "
                         "satisfies fastest, by design) while the duals run "
                         "22-29 and the clipper trains all 30, so any "
                         "comparison also prices a 3-4x compute gap.")
    args = ap.parse_args()

    camp = "headroom_b%d_lrc%g%s%s" % (args.budget, args.lr_constraint,
                                       "_noceskip" if args.no_ce_skip else "",
                                       "_full" if args.full_budget else "")
    ...
    for i, (ds, model, tag, method, seed, extra, ep) in enumerate(todo):
        ...
        if args.no_ce_skip and ep["constraint_epochs"] > 0:
            # Set it UNCONDITIONALLY for every arm that has a constraint phase.
            # Guarding on `"enable_ce_skip" in hp` silently skipped fioretto_ldf
            # and hounie_rcl, which never declare the key and so fell back to
            # the method default of True -- the gate stayed ON for both
            # baselines while TraLO ran with it OFF. The post-hoc arms have
            # constraint_epochs == 0, so the gate cannot fire for them and the
            # key is left off to keep their configs identical to earlier runs.
            hp["enable_ce_skip"] = False
        if args.full_budget:
            # All three trained methods read this key with a default of 5 and
            # break out of the epoch loop once the constraint has held that
            # long. Setting it past the budget removes the exit without
            # touching any method code, so nothing else about the run changes.
            hp["stable_count_threshold"] = args.budget + 1
