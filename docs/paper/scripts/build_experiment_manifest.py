...
FIELDS = ["campaign", "root", "dataset", "model", "method", "cap", "seed",
          ...
          "ece", "sat", "sat_epoch", "flips", "constrained_class", "group_column",
...
def row_for(config_path, root):
    ...
    # Flips are in the config's results block; satisfaction is NOT -- no campaign
    # ever wrote it there, so this silently produced an empty `sat` column for
    # every run in the manifest. It lives in the metrics csv under
    # 'Raw All Satisfied' (see src/training/logging.py). Keep the config lookup
    # as a fallback in case an older campaign did spell it there.
    sat = res.get("constraints_satisfied_natively", res.get("satisfied_natively"))
    if sat is None:
        sat = m.get("Raw All Satisfied")
    if isinstance(sat, bool):
        sat = int(sat)
    flips = res.get("samples_adjusted")
    if flips is None:
        flips = m.get("Flips Required")
    sat_epoch = m.get("Satisfaction Epoch")
