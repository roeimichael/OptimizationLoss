...
def score_run(run_dir, cfg, prefix="final"):
    """Score one run.

    `prefix` selects which prediction artifacts to read. "final" is the
    end-of-run model AFTER the lowest-excess checkpoint restore; "prerestore"
    is the model as training actually left it. They differ whenever the restore
    fires, and for an arm whose treatment is traded away by the restore's own
    selection criterion the restored files can be byte-identical between arms
    while the pre-restore ones are not.
    """
    raw = os.path.join(run_dir, "%s_predictions_raw.csv" % prefix)
    fin = os.path.join(run_dir, "%s_predictions.csv" % prefix)
...
# Methods that never train past warm-up: they solve an allocation on warm-up
# probabilities, so lr_constraint and the CE gate are never read for them.
POSTHOC_METHODS = frozenset({"heuristic", "danits_lp"})


def honours_contract(cfg, hp):
    """True when a run compares an OBJECTIVE rather than a learning rate.

    results/headroom sweeps lr_constraint over {1e-4, 5e-5, 5e-6}. A run whose
    constraint phase is 20x slower than its warm-up is not a weaker objective,
    it is a slower optimizer, and pooling the sweep buries that distinction.
    """
    if cfg.get("methodology") in POSTHOC_METHODS:
        return True
    if hp.get("lr_constraint") != hp.get("lr"):
        return False
    if hp.get("enable_ce_skip") is True:
        return False
    return True


def collect(roots, keep=None, warmup=None):
    """Score every run under `roots`.

    `keep` restricts to a set of (dataset, cap, model) cells -- SEED IS NOT part
    of the key, so all seeds of a wanted cell are kept. Pinning the reference to
    one seed made the bar rest on a single run per cell.

    `warmup` restricts to one warm-up length, and defaults to no restriction.
    It used to be hardcoded to 50, which silently emptied the default corpus:
    results/headroom is the COMPUTE-MATCHED campaign (post-hoc arms at warm-up
    30 + 0 constraint epochs, trained arms at 1 + 29) and contains no warm-up-50
    run at all, so --build-reference could never produce a row.
    """
    rows = []
    for root in roots:
        for cfg_path in glob.glob(root + "/**/config.json", recursive=True):
            ...
            if keep and key not in keep:
                continue
            if warmup is not None and hp.get("warmup_epochs") != warmup:
                continue
            if keep and not honours_contract(cfg, hp):
                continue
...
def main():
    ...
    ap.add_argument("--roots", nargs="*", default=DEFAULT_ROOTS,
                    help="corpus for --build-reference. Defaults to the "
                         "compute-matched campaign this module documents, "
                         "not the frozen warm-up-50 grid.")
    ap.add_argument("--ref-warmup", type=int, default=None,
                    help="restrict the reference to one warm-up length. "
                         "Default: no restriction, which is what the "
                         "compute-matched corpus needs since its post-hoc "
                         "and trained arms sit at different warm-ups by design.")
    args = ap.parse_args()

    if args.build_reference:
        keep = {(d, c, m) for d, c, m, _ in CELLS}
        d = collect(args.roots, keep, warmup=args.ref_warmup)
