"""Generate fig_mechanism for the AAAI paper.
  ...
  - fig_mechanism: empirical trajectories on a ONE-EPOCH-WARMUP probe cell
    ...
    Reads the self-contained copy under docs/paper/data/dynamics/ (see
    docs/paper/data/README_DATA.md for provenance).

Run:  python docs/paper/scripts/make_figs.py
"""
...
from fig_style import apply_style, savefig_dual, C_TRALO, C_FIORETTO
...
def make_mechanism():
    ...
    base = os.path.join(ROOT, "data", "dynamics", "dermmnist")
...
if __name__ == "__main__":
    # fig_loss_shape comes from make_loss_shape_fig.py (2-panel
    # penalty+gradient, no in-plot text). This file emits fig_mechanism only.
