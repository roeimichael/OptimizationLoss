...
- ⛔ **CUTPAIR -- a supervised hinge on TRAINING items at the deployment cut, on an augmentation-trained model -- adds NOTHING over its own null. 2026-09-26, knee, cap 76, seeds 2701-2724, n=24, preregistered, pilot gate passed.** Holm over P1-P3 on capped_first grade-3 F1: cut anchor - shifted anchor +0.37 [-0.35, +1.09]; **cutpair_aug - aug_clip -0.00 [-0.92, +0.92]** (tight: < 0.8 of 76 slots); cutpair_aug - clipper **+3.02 [+1.33, +4.71] (Holm 0.0035)**, which is entirely the AUGMENTATION (aug_clip - tralo_null +3.48 [+1.71, +5.25]). The hinge barely changes who fills the slots (turnover vs aug_clip 0.10 = 0.42x the reseed floor; swaps 50.0% correct-direction, net 0.00). Ensembled (amendment 2) the same: P2-ENS +0.23 [-0.40, +0.86], P3-ENS +2.61. **Same pattern as the fmow2 augmentation cell (augfin): augmentation lifts every arm, TraLO adds ~0 over the augmented clipper.** Training labels at the cut are CLOSED as a who-source on this cell. The fair post-hoc bar is the clipper trained with the same augmentation, ensembled. Record: `experiments/claude_cutpair_cap76_result_20260926.md` on `claude/bandcons-20260926`.
