...
`scripts/` are all imported by 400 tests. `main.py` runs every campaign.
...
six restored baselines, six new gate scripts, and 400 tests. **Do not quote a line count as a
...
no cross-objective warm-up sharing), and `pytest tests` (400 tests, ~180 s, no dataset needed).
...
tests/             400 tests, ~180 s, no dataset required
