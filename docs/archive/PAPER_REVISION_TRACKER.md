...
> A point-in-time review tracker. The paper of record is now `docs/paper/main_edited_by_roei.tex`.
...
# Round 2 — blind re-review, 2026-08-13/14

Full report and my verification notes: `docs/review/round2_blind_2026-08-13.md`.
Score **71/100, Major Revision** (round 1 was 58). The reviewer had no knowledge
of round 1 or of anything changed since.

## Applied (all with red/blue marks, both PDFs rebuilt clean: rev 36pp, clean 35pp)

- **B3** Table 4 caption: "the second row is the joint removal" was wrong (it is
  the third), and "It helps in 22 of 24" had no antecedent. Both fixed.
- **C1** Algorithm 1 line 3 said the hinge engages "once satisfied". The code
  gates it on `lambda>0` only (`tralo/train.py:308-332`), so it is carried from
  the first constraint epoch and is zero by `relu` while the soft count is at or
  above the cap. Wording fix, not a re-run; Sec. 3's prose was already right.
- **B4** Sec. 3's "essentially never needed" now scoped to the *global* cap, with
  the joint figures (0.21 / 0.60) stated inline instead of forward-referenced.
- **B5** "the lone exception" was two, both Hounie-RCL on OctMNIST (+0.000
  RegNetY-400MF, -0.003 ViT-B/16). Recomputed and corrected. TraLO clears the
  best clipper in all nine cells, which the new sentence now says.
- **F1** abstract scoped to the global cap.
- **App. B.3** "+0.088" was wrong; the 30 arm-by-cell margins run +0.0144 to
  +0.0828. Now +0.083.
- **D1 + C2** one new blue Sec. 4 paragraph disclosing (a) that `force_exact`
  budget-fills the trained family but not LP-LG, and (b) that `ce_nan` is a
  logging artifact of the shared CE-saturation gate, not divergence.
- **A4** the LOBO reference said "script in the released corpus", which sent the
  reviewer to the data directory. Now names `scripts/lobo_cv.py` and explains the
  per-fold band selection.
- **F2** seven lost inter-word spaces. Root cause was `strip_revision_marks.py`
  removing `\color{blue}\s*`, which ate the deliberate space in
  `training{\color{blue} with ...}`. Regex now consumes no whitespace.
  `scripts/check_lost_spaces.py` guards it and runs inside `build_clean.py`;
  it is a *difference* test (marked vs clean), because `RegNet{Y-400MF}` is an
  intended join and a bare pattern match is all false positives.

## Released (closes A1, A2, A5; `release_manifest_campaigns.py`)

| file | runs | verified |
|---|---|---|
| `corpus/imbalanced_baselines.csv` | 312 | Table 3 reproduces **exactly, all 27 cells** |
| `corpus/native224_ham10000.csv` | 96 | App. D (i) +0.013 and (ii) -0.001/+0.001 exact |
| `corpus/ablation_no_hinge.csv` | 24 | +0.0323, 22/24, 6/6 cells — matches Table 4 |
| `corpus/mnv2_provenance.csv` | 148 cells | 132 single-campaign; **OctMNIST 0/30 multi** |

## Refuted — five reviewer claims that do not survive checking

- **A3** (-4) "no `no_hinge` arm anywhere". The runs exist (`g5_hinge_oct`) and
  reproduce exactly. It was a release gap, now closed. The label collision
  (`ablation_complete.csv` calls the +0.036 row `hinge`) is real.
- **A4** (-1) "LOBO not reproducible, script not released".
  `paper/scripts/lobo_cv.py` runs and gives **+0.0328 at 92.9%**, 100% on the
  tight caps — the paper's +0.033/93%. The reviewer looked for scripts in
  `data/corpus/`. Their +0.0377/100% is the fixed six-cell mean; LOBO selects the
  band per fold, and holding out MobileNetV3 admits L60 (n=28, not 24).
- **A5** (-1) provenance "unauditable". The `sweep` column always carried it; it
  was only unsummarised. Now published per cell.
- **B1** (-2) "Table 9's counts are wrong". Table 9 reproduces exactly under its
  own caption rule. What is real: `tab_granular_oct.tex`'s `win` column is a
  plain **sign** winrate (`make_granular_tables.py:48`), Table 9's is a
  clear-the-band winrate, and both are labelled "win". **OPEN — see below.**
- **E1** (-1) "an n=4 percentile CI excludes zero exactly when all four gaps
  share a sign". False: `[.05 .04 .06 -0.0001]` excludes zero, `[... -0.02]`
  does not. The coverage caveat stands; the sign-test equivalence does not.

## Traps found this round (do not repeat)

- **`b1` sweeps warmup {1, 5, 50}.** Averaging over it flips two Table 3 cells,
  including Derm x MobileNetV3 from -0.012 to +0.041. Filter `warmup == 50`.
- **`g5_component_ablation` and `component_ablation` are unusable as controls.**
  Five of six removal arms land within 0.005 of `full`, including `no_reset`,
  which the released `B_loo_ablation` block puts at +0.079. Only `no_warmup`
  moved (0.26 vs 0.37). The flags did not take effect. `verify_hinge_arm.py`
  tests for this before quoting anything from that campaign.
- App. D claim (ii) is **cc-F1**, not macro-F1. Reading the wrong column puts
  Hounie at +0.002 and makes the paper look off by a digit.

## Open — needs a decision

1. **B1 rule.** One win rule, applied in both tables. Table 9's (>=half clear
   +0.005) gives RegNet x Oct 4W/5T/0L; the granular rule (>=3/4 positive) gives
   5W; strictest (>=3/4 clear +0.005) gives 3W. Cell *means* agree throughout.
2. **C2 matched-count re-run** of trained-vs-LP-LG macro-F1. Disclosure is
   written either way. `docs/AUDIT_2026-07-31.md` shows equalising moved the
   cc-F1 headline +0.038 -> -0.002.
3. **Hinge caveat.** Table 4's hinge row pairs `g5_hinge_oct` against
   `paper_final` *across campaigns* and no drift control exists (the two share no
   arm). It rests on 6/6 cells and 22/24 runs over 3 backbones and 2 caps.
   State it or not — it is the row carrying "each load-bearing on its own".
4. **D2 strengthening.** Block C is MobileNetV3-only (true), and Hounie is far
   more eta-sensitive than the paper implies (0.3861 at eta=0.001 vs ~0.29 at
   0.05/0.1, L30). But **even at its best eta Hounie still trails Fioretto**
   (0.3913/0.4874), which is the per-seed-best comparator — so tuning cannot
   move it. Fioretto's step size is inert to four decimals, corroborating the
   Adam scale-invariance mechanism.

## Still unverified

- **E2** (-1) the six cells share seeds and backbones, effective n nearer 3.
  Judged correct; it is a design argument, not a checkable claim.
- App. D claim (iii) and Sec. 5.3's "1.0 vs 0.0" need `sat`, which the manifest
  does not carry for `b1` or `b2_derm`. Server-side copy, not a re-run.
