...
> ⚠️ **This page is written as an onboarding brief for an AI assistant, which makes it the single most dangerous file in the repo to read casually.** It teaches **warm-up 50** (a dead regime -- CE saturates and every method becomes identical) and the **`alpha_kl` KL anchor**, which is DELETED from the pipeline. It also names dermmnist/octmnist/tissuemnist as live datasets; all three are REMOVED.
