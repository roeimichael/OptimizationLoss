...
{\small Prepared 9 August 2026, updated 13 August \quad$\cdot$\quad read this one document; everything else is backup}
\end{center}
...
\texttt{tralo\_bounded/train.py} contains \emph{zero} occurrences of the
...
more cannot hurt less. Row relabelled \textbf{``Reset $+$ hinge (TraLO-bounded)''}, and a
new row \textbf{``Undershoot hinge (alone)''} was added underneath it, from 24 runs
dispatched and completed on 2026-08-13 --- so the hinge is now isolated on the very cells
that carry the claim. It costs $+0.032$ cc-F1, winrate $0.92$, positive in all six cells.
\item \textbf{The caption called itself ``Complete'' and was not.}
\texttt{ablation\_complete.csv} has five arms at $n{=}24$; the table showed four. The
omitted one is \texttt{+KL} (mean $-0.0103$, $p{=}0.0398$) --- significant, but it is an
\emph{addition}, not a removal, and it adds a term the paper does not use
($\alpha_{\mathrm{KL}}{=}0$ corpus-wide). Rather than show a row for a variant that is not
the method, the word ``Complete'' was struck from the caption, and the orphaned KL sweep
paragraph in Appendix B --- which described the same dead term --- was struck with it.
\emph{If he asks why the KL result is not shown: because it argues about a hyperparameter
we set to zero, and the honest fix is to stop claiming completeness, not to advertise an
arm we do not run.}
\end{itemize}
...
\subsection{``Is the hinge doing anything on its own, or only alongside the reset?''}
\label{sec:hingealone}
On its own. This was the one genuine hole in the paper's own framing, and it was closed the
night before this meeting with 24 fresh runs.

\smallskip
The arm is \texttt{hybrid\_mode=bounded\_only} with \texttt{reset\_optimizer\_at\_sat=True}
--- the hinge removed, the reset \emph{retained} --- on exactly the grid Table 5 uses:
OctMNIST $\times$ $\{L30,L40\}$ $\times$ $\{$MobileNetV3, RegNetY400MF, ViT-B/16$\}$
$\times$ 4 seeds. All 24 completed, zero failures.
...
\midrule
\multicolumn{3}{l}{\emph{overall} $+0.032$, helps in 22 of 24 runs, 6/6 cells positive} \\
\bottomrule
\end{tabular}
\end{center}

\noindent Every cell is positive, so the hinge clears the same cell-level sign test as the
reset ($p{=}0.031$, the floor at six cells). \textbf{Both} named components are now
load-bearing individually, which is what the title claims.

\smallskip
The one number worth being ready for: reset alone $+0.079$, hinge alone $+0.032$, both
together only $+0.036$. That looks wrong until you say it out loud --- \emph{the two are
not additive}. They fix overlapping failure modes, so removing both costs less than the sum
of removing each. The caption now says this explicitly rather than leaving him to notice it.
...
\item \textbf{\textcolor{revred}{[CLOSED 13 Aug]} Only one of the ``two portable
components'' was isolated on the cells that carry the claim.} This was the likeliest place
for a reviewer to press. The missing 24 runs were dispatched and completed, and they
settled it \emph{in the paper's favour} --- numbers in \S\ref{sec:hingealone} above.
