...
✅ **AND STEP 5 HAS RUN.** The standing figure is **8 of 27 = 30%**
(strict-task **6 of 18 = 33%**, per unit **2 of 11**), bar 50%, VERDICT FAIL,
measured 2026-09-11 on the fixed tool over the twelve licensed-unit campaigns.
`7 of 25 = 28%` (pre-G1), `7 of 23 = 30%` (pre-F1), `6 of 22 = 27%` and the
never-run `6 of 24 = 25%` are all superseded. **3 cells PRICED, tralo wins 1.**
...
rows were not measurements. Both were fixed from the TRACKED `*_meta.csv`, on
CPU, in seconds -- no images, no GPU, no SSH.

| slice | was | is | how it was settled |
|---|---|---|---|
| `bcn/oodslice` | `-- / -- / --` | **+2031 / 61.6 / 8** | never screened at all |
| `fmow/oodslice` | +2969 / 79.7 / **10** | **+2793 / 76.2 / 13** | three authorities against the row |
| `iwildcam/oodslice` | +3133 / 96.3 / 7 | **unchanged, byte-identical** | the control that the tool did not move |

🔑 **THE GROUP COUNT IS THE FIELD THAT SETTLES IT** -- but by
CONTEMPORANEOUS RECORD, not by independent measurement, and the first draft of
this section got that wrong. `configs/task_windows.yml` line 267 says
`1 of 13`, `tier_viability` says `grp 13`, and the meta holds 13 countries /
4168 items / ZERO overlap vs 136 train countries -- **all three read the same
file**, as the yml's own header admits ("Counted from the test labels alone").
⚠️ The yml block has TWO provenances: its WINDOW rows come from
`fmow1`'s own nulls, its GROUP-COUNT table from labels. What defeats `10` is
that the yml was written WHILE `fmow1` ran and revised when it finished, so it
records which slice was in play. ⛔ Whether the SERVER worktree holds this
same slice is one md5 per file and is task **#134**. No size filter explains
13 -> 10 either -- the smallest group holds **136** items.

⛔ **HALF OF bcn's NOVELTY IS INTERPOLABLE, AND THAT WAS MEASURED ON
2026-09-01 AND NEVER REACHED A DECISION.** 2(w2c) already records `bcn_s2` at
**50.4** among the factorial rows that reproduce exactly. bcn's group is
`anterior torso|40s`, a PRODUCT of two factors both present in training, so
`dataset_screen`'s baseline (hand an unseen group the global training prevalence)
is TOO GENEROUS -- the model can interpolate site x age. The figure existed while
bcn's table row stayed three dashes, `bcn1mn3` ran 228 runs and unit D1 entered
the ledger. **The defect is a measurement with no path to the decision it bears
on.** Now read directly off the DEPLOYED slice, raked **8 of 8**:
...
iwildcam and every fmow slice rake **0** (atomic labels). ⛔ That is NOT the
same as "the tool never ran": 2(w2c) lists five candidate slices it raked, and
`bcn_s2` at **50.4** is among them. What is new here is that the figure is now
read off the slice a CAMPAIGN actually used, and that it had never reached the
table it bears on.
...
untouched in DIRECTION: **8 of 27 = 30%, strict-task 6 of 18 = 33%, per unit
2 of 11, FAIL** (recomputed three times on 2026-09-11 as D2, F1 then G1 were
licensed; was `6 of 22 = 27%, per unit 2 of 8`).
...
these groups and corrects the raked baseline twice. "About half" is the
defensible sentence; `50.4%` should not be quoted to three figures.

🔑 **THE RULE: AN EMPTY CELL IN A DECISION TABLE IS INVISIBLE TO EVERY
AUDIT THAT READS THE TABLE**, because there is nothing there to be wrong. 2(z83)
found the inverted form (a campaign with no row at all) and the remedy is the
same -- harvest the subjects from the authorities, not from the table's contents.
Gated: `test_the_dataset_table_unseen_group_counts_match_the_TRACKED_meta`,
mutation-tested **5/5**, including the exact live defect (fmow=10) and the
verified-NOTHING case. FRAMEWORK **2(z104)**. Tasks **#134** (confirm the server
fmow slice by md5) and **#135** (decide `bcn1vit`).
...
* ✅ **RUN 2026-09-11, AND THE DERIVATION WAS WRONG IN BOTH HALVES.** The
  ...
  **8 of 27 = 30%**, strict-`task` **6 of 18 = 33%**, **per unit 2 of 11**, bar
  50%, VERDICT FAIL (that is with F1 and G1 added hours later; the first run of
  the day read 7 of 23 = 30% / 2 of 9, then 7 of 25 = 28% / 2 of 10). The derivation missed two things it could not have known:
