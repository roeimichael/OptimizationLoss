...
caption now states that the hinge's individual contribution is not identified here.
...
\item \textbf{Only one of the ``two portable components'' is cleanly isolated.} Following
from the Table 5 correction above: the reset is isolated singly, the hinge is not. A
correctly isolated arm exists in the codebase
(\texttt{gen\_g5\_component\_ablation.py}, the \texttt{bounded\_only} configuration, which
keeps the reset) but has not been run at this scale. Since ``two portable components'' is
billed as the paper's central mechanistic finding, this is the most likely place for a
reviewer to press. \emph{Cost to settle it: one ablation arm, 24 runs.}
