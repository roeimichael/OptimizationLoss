...
| `bcn1mn3` | **COMPLETE 228** | MobileNetV3 | `L70` `L80` `L90` | 🟢 **THE SECOND DATASET.** 3 caps x 19 arms x 4 seeds on `bcn/oodslice`. ⛔ `L70` is a measured NON-TASK -- nothing there is evidence about any method (2(z58)). This is unit **D1** and its SIGN IS UNREAD; reading it costs zero GPU-hours (0-UNREAD, task #102) |
| `bcn1vit` | **COMPLETE 190/190, UNLICENSED** (was "live" until 2026-09-11 -- 2(z106)), **L90 only** | **ViTB16** | `L90` | `L70` and `L80` were archived as non-task to `~/optloss-archive-bcn1vit-L70-nontask-2026-09-09` (moved, not deleted). L90 is a task cell under both readings -- finish it. 2(z58) |
| `fmow1` | live, **>=114/304 as of 2026-09-09** | 2 backbones | 2 caps | 🟢 **THE THIRD DATASET, and the first campaign in this project's history whose caps landed inside a MEASURED task window** -- 3 of its 4 (backbone, cap) cells are TASK cells, classes 3 and 5. Local p@K 0.842/0.882/0.952/0.973 against iwildcam's 0.9948-0.9972. Re-measure the windows at 4 seeds on completion, task #96. 2(z59) |
| `vitdual2` | **>=32/88 as of 2026-09-06** | **ViTB16** | `L80-80_G95` `L90-90_G95` | **4 duals** + per-family nulls + clippers, equal dose. ⚠️ This row read `RUNNING 0/88` until 2026-09-10, which was false in both halves: the campaign has produced the ViTB16 `headroom` rows (13.0 / 20.7 items) and `deployed_h2h` has read its L90-90 cell -- the ONLY cell with all four duals at equal dose. **Run-state lives in MISSION `0-RUNNING`, not here**; this column is a coverage note and carries the date it was last checked. FRAMEWORK 2(z72) |
