#!/usr/bin/env bash
# =============================================================================
#  results/dom1b  --  THE RegNetY400MF HALF OF dom1. Scored WITH it.
# =============================================================================
#
...
#
#   size        3 cells x 16 arms x 4 seeds = 192 runs on THIS host.
#               🛑 THREE CELLS CANNOT CALL ANYTHING ALONE (exact floor 0.25).
#               This root exists ONLY to be scored WITH `dom1`:
#                   full_panel --campaign results/dom1 results/dom1b ...
#               which is 9 cells, the minimum for a `***` verdict. Both roots
#               pin the SAME commit or the join is a split code_version and
#               full_panel REFUSES it.
...
#               with --constraint-fp32, which iwc4 verified removes the
#               FP16 dose loss entirely (1044/1044 over 36 runs).
#
#   read it     python -m scripts.rig_status
#               python -m scripts.dose_landed results/dom1b
...
#               python -m scripts.full_panel --campaign results/dom1b results/dom1bb --control clip
#               python -m scripts.full_panel --campaign results/dom1b results/dom1bb --control tralo_null
...
TREE=~/optloss-domb           # its OWN worktree. Worktrees share one object
...
#
#     git show origin/headroom/small-cnn:docs/launch_dom1b.sh > ~/launch_dom1b.sh
#     GPU=0 bash ~/launch_dom1b.sh
#
...
echo "dom1 holds the other on dsisco02: this is the second and last."
