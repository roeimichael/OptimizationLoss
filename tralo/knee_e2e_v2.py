"""End-to-end knee study that fixes audit findings D1 and D3/D4 and adds the sham control.

Usage: python -m tralo.knee_e2e_v2 DATA_ROOT CONFIG_JSON OUTPUT_DIRECTORY [ARM ...]

Protocol: experiments/claude_controller_sham_protocol_20260925.md (fixed before launch).
...
Five arms share initialisation, warm-up and minibatch order (hash-checked):
  ...
  tralo_adam   the published arm: separate Adam constraint step (controller inert, D1)
  tralo_sgd    CalibratedSGD constraint step: first step matches the Adam dose,
               later steps scale with multiplier x violation (controller live)
  sham_sgd     ShamOptimizer over CalibratedSGD: identical per-tensor step norm at
               every step, seeded random direction -- the perturbation control
The backbone trains (D4: a frozen head can only re-rank by feature L1 norm), and the
grade-3 cap of 76 binds on the hard count (D3), which the runner records per seed.
...
from .constraint_optimizers import CalibratedSGD, ShamOptimizer
...
CONSTRAINED = ('tralo_adam', 'tralo_sgd', 'sham_sgd')
SEEDS = tuple(range(1701, 1725))
...
def validate(config):
    ...
    if config['seed'] not in SEEDS:
        raise ValueError('seed is outside the preregistered set 1701-1724')
...
def constraint_optimizer(arm, model, config):
    ...
    if arm == 'tralo_sgd':
        return CalibratedSGD(params, adam_lr=config['constraint_lr'])
    if arm == 'sham_sgd':
        return ShamOptimizer(CalibratedSGD(params, adam_lr=config['constraint_lr']), seed=config['seed'] + 7)
    return None
...
def train_one(model, train_images, val_chunks, config, arm, emit, snapshot):
    ...
    return dict(warmup_sha256=warmup, batch_sha256=batch_hash.hexdigest(),
                ...
                parameter_displacements=displacements,
