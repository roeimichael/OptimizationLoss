...
def main():
    ...
    print("\n" + "="*80)
    print("ENHANCED MODEL EXPERIMENTS")
    print(f"Model Type: {MODEL_PARAMS['model_type']}")
    print(f"  Use Residual: {MODEL_PARAMS['use_residual']}")
    print(f"  Use Attention: {MODEL_PARAMS['use_attention']}")
    ...
    for config_idx, config in enumerate(NN_CONFIGS):
        ...
        for constraint_pair in CONSTRAINTS:
            ...
            print(f"  Training with transductive approach (enhanced model)...")
            ...
            benchmark_metrics_path = f"{RESULTS_DIR}/{constraint_folder}/hyperparam_{config_name}/benchmark_metrics.csv"
            benchmark_metrics = {}
