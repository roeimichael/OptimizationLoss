...
def main(model_variant='baseline', model_params=None):
    """
    Run experiments with specified model variant.

    Args:
        model_variant: Name of model variant (e.g., 'baseline', 'enhanced_gelu_residual', 'enhanced_attention')
        model_params: Dictionary of additional model parameters (e.g., {'use_residual': True, 'activation': 'gelu'})
    """
    ...
    results_base_dir = Path(RESULTS_DIR) / model_variant
    ...
    for config_idx, config in enumerate(NN_CONFIGS):
        ...
        for constraint_pair in CONSTRAINTS:
            ...
            results_file = f"{results_base_dir}/{constraint_folder}/students__{config_name}__transductive.csv"
            os.makedirs(f"{results_base_dir}/{constraint_folder}", exist_ok=True)
            ...
            benchmark_metrics_path = f"{results_base_dir}/{constraint_folder}/hyperparam_{config_name}/benchmark_metrics.csv"
            benchmark_metrics = {}
    ...
    results_json_path = f"{results_base_dir}/nn_results.json"
    ...
    print(f"\n{'='*80}")
    print(f"Experiment complete for model variant: {model_variant}")
    print(f"{'='*80}\n")
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description='Run experiments with different model variants')
    parser.add_argument('--variant', type=str, default='baseline',
                       help='Model variant name (e.g., baseline, enhanced_gelu_residual, enhanced_attention)')
    ...
    model_params = {}
    if args.use_residual or args.use_attention or args.activation != 'relu':
        model_params['use_residual'] = args.use_residual
        model_params['use_attention'] = args.use_attention
        model_params['activation'] = args.activation

    main(model_variant=args.variant, model_params=model_params if model_params else None)
