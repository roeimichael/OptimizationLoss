"""Prepare BloodMNIST into the slice format used by the pipeline loader.

Mirrors data/pathmnist/prepare_pathmnist.py:
...
BloodMNIST (MedMNIST v2): 17,092 normal blood-cell images, 28x28 RGB, 8 classes.
We use full train (~12k, < 15k cap so all kept), stratified subsample test->3k,
upscale 28->224 (LANCZOS, 3-channel preserved).
Constrained class = basophil (0, rarest class) -> the "cap basophil predictions
to the lab's capacity" story (most-imbalanced cell type).
Group = synthetic balanced 2-split (no real metadata in MedMNIST).

Run on the server in data/bloodmnist/ after bloodmnist.npz is downloaded
(handled by download_data.py).
"""
import os
import numpy as np
...
NPZ = os.path.join(DATA_DIR, 'bloodmnist.npz')
...
CLASS_NAMES = {
    0: 'BASO',   # basophil (constrained, rarest)
    1: 'EOSI',   # eosinophil
    2: 'ERYT',   # erythroblast
...
def main():
    ...
    print(f"\nDone -> {out}/  (constrained class BASO={CONSTRAINED}, "
