#!/bin/bash
# claude_cutpair_queue.sh <gpu> <seed> [<seed> ...]
...
OUT=/home/dsi/michaer8/tralo-rebuild/runs/claude-cutpair-${BLOCK:?set BLOCK}
...
for SEED in "$@"; do
  ...
  say "START seed $SEED"
  CUDA_VISIBLE_DEVICES="$GPU" "$PY" -u -m tralo.knee_e2e_v5 "$DATA" \
    "experiments/configs/claude_cutpair_$SEED.json" "$OUT/seed$SEED" > "$OUT/seed$SEED.log" 2>&1 < /dev/null
