#!/bin/bash
# claude_sham_queue.sh <gpu> <seed> [<seed> ...]
...
for SEED in "$@"; do
  ...
  say "START seed $SEED"
  CUDA_VISIBLE_DEVICES="$GPU" "$PY" -u -m tralo.knee_e2e_v2 "$DATA" \
    "experiments/configs/claude_sham_$SEED.json" "$OUT/seed$SEED" > "$OUT/seed$SEED.log" 2>&1 < /dev/null
