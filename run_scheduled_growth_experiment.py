#!/usr/bin/env python3
"""
Runner script for scheduled growth with loss gates experiments.

This script runs a single experiment using the scheduled growth methodology
where lambda only increases when constraint loss fails to improve.
"""
...
def run_scheduled_growth_experiment(config_path: str) -> Optional[Dict[str, Any]]:
    """
    Run a single scheduled growth adaptive lambda experiment.
    ...
    print(f"\n{'='*80}")
    print(f"RUNNING SCHEDULED GROWTH EXPERIMENT")
    print(f"{'='*80}")
    print(f"Experiment path: {experiment_path}")
    print(f"Model: {config['model_name']}")
    print(f"Constraints: Global {config['global_constraints']}, Local {config['local_constraints']}")
    print(f"Growth factor: {config.get('growth_factor', 1.1)}")
    print(f"Check frequency: {config.get('check_frequency', 10)} epochs")
    print(f"{'='*80}\n")
